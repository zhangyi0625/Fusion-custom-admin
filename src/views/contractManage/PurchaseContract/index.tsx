...
} from '../SalesContract/AddSalesContract'
...
const PurchaseContract: React.FC = () => {
  ...
  const tableColumns: TableProps['columns'] = [
    ...
    {
      title: '已收票金额',
      key: 'receiveInvoicePrice',
      dataIndex: 'receiveInvoicePrice',
      align: 'center',
      width: 100,
    },
    {
  ...
  const loadSearchList = () => {
    Promise.all([getRoleUser({}), getCustomerList({}), getSupplier({})]).then(
      (resp) => {
  ...
  const getReduxData = () => {
    let { customerData, supplierData } = essential
    searchColumns.map((item) => {
      if (item.name === 'customerId' || item.name === 'supplierId') {
        item.options = item.name === 'customerId' ? customerData : supplierData
      }
    })
